...
class S3(CloudUpload):
    ...
    async def upload(self, *, file: UploadFile, url_s3: str) -> FileData:
        ...
        """
        try:
            filesystem_type, bucket_s3, key_s3 = self.parse_url_s3_as_localisation(s=url_s3)
            key_s3 = key_s3[1:]
    ...
    def smart_read_xyz(self, s3_url: str):
        try:
            ...
            # extra_args = self.config.get("extra_args", {})
            with smart_open.open(s3_url, 'r', transport_params=dict(client=self.client)) as fin:
                match s3_url_suffix:
                    case '.pts':
    ...
    def smart_read_ply(self, s3_url, allow_bool=False):
        ...
        with smart_open.open(s3_url, 'rb', transport_params=dict(client=self.client)) as ply:
        ...
        if fmt == 'ascii':
            ...
            with smart_open.open(s3_url, 'rb', transport_params=dict(client=self.client)) as dply:
                data["points"] = pd.read_csv(dply, sep=" ", header=None, engine="python",
            ...
            if mesh_size :
                ...
                data["mesh"] = pd.read_csv(
                    s3_url, sep=" ", header=None, engine="python", skiprows=top, usecols=usecols, names=names)
            ...
            with smart_open.open(s3_url, 'rb', transport_params=dict(client=self.client)) as ply:
                ...
                points_np = np.frombuffer(ply.read(), dtype=dtypes["vertex"], count=points_size)
